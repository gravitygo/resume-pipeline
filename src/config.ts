...
export function loadConfig(overrides: Partial<Config> = {}): Config {
  ...
  if (!["openai", "azure"].includes(aiProvider)) {
    throw new Error(
      `Invalid AI_PROVIDER: ${aiProvider}. Must be 'openai' or 'azure'`,
    );
  }
  ...
  if (aiProvider === "openai") {
    ...
    if (!endpoint) {
      throw new Error(
        "AZURE_OPENAI_ENDPOINT or AZURE_AI_ENDPOINT environment variable is required when using Azure provider",
      );
    }
    if (!apiKey) {
      throw new Error(
        "AZURE_OPENAI_API_KEY or AZURE_AI_API_KEY environment variable is required when using Azure provider",
      );
    }
    if (!deploymentName) {
      throw new Error(
        "AZURE_OPENAI_DEPLOYMENT or AZURE_AI_DEPLOYMENT environment variable is required when using Azure provider",
      );
    }
  ...
  return {
    ...
    openaiApiKey,
    openaiModel:
      overrides.openaiModel || process.env.OPENAI_MODEL || "gpt-4o-mini",
    azureConfig,
